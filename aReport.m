REPORT=load("reports\REPORT_backup.mat");
clear sumMatrix
sumMatrix = zeros(10,"int32");
fields = string(fieldnames(REPORT)');

%output matrices and relative filters
nofilterMatrix = zeros(10,"int32");
nofilter = @(x) contains(x,'f0');
filter1Matrix = zeros(10,"int32");
filter1 = @(x) contains(x,'f1');
filter3Matrix = zeros(10,"int32");
...
resol50Matrix = zeros(10,"int32");
resol50 = @(x) contains(x,'d5000');
segm1Matrix = zeros(10,"int32");
segm1 = @(x) contains(x,'s1c');
segm2Matrix = zeros(10,"int32");
segm2 = @(x) contains(x,'s2');
segm5Matrix = zeros(10,"int32");
segm5 = @(x) contains(x,'s5');
segm10Matrix = zeros(10,"int32");
segm10 = @(x) contains(x,'s10');
coher1Matrix = zeros(10,"int32");
coher1 = @(x) contains(x,'c1');
coher3Matrix = zeros(10,"int32");
coher3 = @(x) contains(x,'c3');
coher5Matrix = zeros(10,"int32");
...
for sec = fields
    sumMatrix = sumMatrix + int32(REPORT.(sec));
    if nofilter(sec) nofilterMatrix = nofilterMatrix + int32(REPORT.(sec)); end
    ...
    if segm1(sec) segm1Matrix = segm1Matrix + int32(REPORT.(sec)); end
    if segm2(sec) segm2Matrix = segm2Matrix + int32(REPORT.(sec)); end
    if segm5(sec) segm5Matrix = segm5Matrix + int32(REPORT.(sec)); end
    if segm10(sec) segm10Matrix = segm10Matrix + int32(REPORT.(sec)); end
    if coher1(sec) coher1Matrix = coher1Matrix + int32(REPORT.(sec)); end
    if coher3(sec) coher3Matrix = coher3Matrix + int32(REPORT.(sec)); end
    if coher5(sec) coher5Matrix = coher5Matrix + int32(REPORT.(sec)); end
end
figure(10)
